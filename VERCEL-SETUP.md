...
## 🔒 Setup: Template → Vercel → Private

### Step 1 — Use the template (not Fork)

Click **"Use this template"** on the StorefrontKit repo → create your own repo.

⚠️ Don't use GitHub's **"Fork"** button. A fork stays tied to the original repo's network, and GitHub won't let you flip a fork to private on its own — you'd need to detach it first, which is a hassle. **"Use this template"** instead creates a brand-new, fully independent repo that you can set to private immediately, which is the whole point of this guide.
