...
No coding needed. Paste this prompt into Claude, ChatGPT, or any AI to fully customize your store design and settings.
...
BUSINESS & LEGAL (powers About, Services, Privacy, Terms pages):
- Legal/business name: [e.g. Acme Goods LLC]
- Address: [optional, shown on legal pages if provided]
- Support email: [email address]
- Support phone: [phone number]
- Returns window: [e.g. 30 days]
- Shipping note: [e.g. "Orders processed within 1-2 business days"]
- Our story (About page): [a short paragraph about why your store exists]
...
| Enable announcement bar | Banner at top of every page |
| Business/legal details | About, Services, Privacy, and Terms pages |
| Enable/disable a payment method | Shows/hides that option at checkout (`payments.methods`) |
| Add Stripe/PayPal/Razorpay keys | Activates that processor once its toggle is on |
| Congrats heading/message/image/video/audio | The order-confirmation screen after checkout |

Orders themselves (customer names, addresses, order status) live in `orders.json`, not `config.json` — manage those in `/admin.html → Orders`, not through the AI.
...
2. Click `config.json` → pencil icon
3. Select all → paste new JSON
4. Click **Commit changes**
5. Store updates in ~30 seconds (GitHub Pages) or ~20 seconds (Vercel)
...
6. Click **Add Product** — it saves to GitHub automatically
...
StorefrontKit ships with 7 templates (see [README](./README.md) for previews). Each follows the same URL pattern — template 1 has no number, templates 2-7 add the number to every filename:

| Page | Template 1 | Templates 2-7 |
|---|---|---|
| Homepage | `/index.html` | `/index2.html` ... `/index7.html` |
| Product Detail | `/product.html?id=XXX` | `/product2.html?id=XXX` ... |
| Cart | `/cart.html` | `/cart2.html` ... |
| Checkout | `/checkout.html` | `/checkout2.html` ... |

Shared across every template (no number, used regardless of which template you keep):

| Page | URL | Purpose |
|---|---|---|
| About | `/about.html` | Your story, values, mission |
| Services | `/services.html` | Shipping, returns, payment, support info |
| Privacy Policy | `/privacy.html` | Auto-filled from your business/legal config |
| Terms of Service | `/terms.html` | Auto-filled from your business/legal config |
| Admin | `/admin.html` | Add/manage products |
| Setup | `/setup.html` | 8-step wizard: identity, template, colors, story, business/legal, contact, API keys, hero |

Pick one template, delete the rest of the numbered files — About/Services/Privacy/Terms and the admin/setup tools stay exactly as they are either way.
